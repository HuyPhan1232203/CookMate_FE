...
const BlogPage = () => {
  ...
  return (
    <div style={{ padding: "60px 0", background: "#f8f9fa", minHeight: "100vh" }}>
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        {loading ? (
          ...
          <Alert type="error" message={error} showIcon style={{ marginBottom: 32 }} />
        ) : posts.length === 0 ? (
          <Empty description="Chưa có bài viết nào" style={{ margin: "60px 0" }} />
        ) : (
          <Row gutter={[32, 32]}>
            {posts.map((post) => (
              <Col xs={24} md={12} key={post.id}>
                ...
                >
                  ...
                  <Paragraph ellipsis={{ rows: 2 }}>{post.content || post.description || ""}</Paragraph>
                  <div style={{ margin: "12px 0" }}>
                    {/* Nếu có tags thì hiển thị, nếu không thì bỏ qua */}
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 8 }}>
                    <Avatar size={32} icon={<UserOutlined />} style={{ background: "#ff8c42" }} />
