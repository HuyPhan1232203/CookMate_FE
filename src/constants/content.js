// Content constants for CookMate application
export const APP_CONTENT = {
  // App Name
  APP_NAME: "CookMate",

  // Header
  HEADER: {
    MENU_ITEMS: {
      RECIPES: "Công thức",
      FAVORITES: "Yêu thích",
      SEARCH: "Tìm kiếm",
    },
    BUTTONS: {
      LOGIN: "Đăng nhập",
      REGISTER: "Đăng ký",
    },
  },

  // Hero Section
  // HERO: {
  //   TITLE: "Nấu ăn thông minh với",
  //   TITLE_HIGHLIGHT: "CookMate AI",
  //   SUBTITLE:
  //     "Khám phá hàng ngàn công thức nấu ăn phù hợp với nguyên liệu bạn có. Hệ thống AI thông minh sẽ gợi ý những món ăn ngon nhất cho bạn!",
  //   BUTTONS: {
  //     PRIMARY: "Tìm công thức ngay",
  //     SECONDARY: "Xem thư viện công thức",
  //   },
  //   STATS: {
  //     RECIPES: {
  //       NUMBER: "1000+",
  //       LABEL: "Công thức",
  //     },
  //     USERS: {
  //       NUMBER: "50K+",
  //       LABEL: "Người dùng",
  //     },
  //     RATING: {
  //       NUMBER: "4.8★",
  //       LABEL: "Đánh giá",
  //     },
  //   },
  // },

  // Features Section
  // FEATURES: {
  //   TITLE: "✨ Tính năng nổi bật",
  //   SUBTITLE:
  //     "CookMate mang đến cho bạn trải nghiệm nấu ăn hoàn toàn mới với công nghệ AI tiên tiến",
  //   ITEMS: [
  //     {
  //       title: "AI Thông Minh",
  //       description:
  //         "Hệ thống AI tiên tiến phân tích nguyên liệu và gợi ý công thức phù hợp nhất với sở thích của bạn.",
  //     },
  //     {
  //       title: "Cộng đồng yêu thích",
  //       description:
  //         "Kết nối với hàng ngàn người yêu thích nấu ăn, chia sẻ công thức và học hỏi kinh nghiệm.",
  //     },
  //     {
  //       title: "Tiết kiệm thời gian",
  //       description:
  //         "Nhanh chóng tìm ra món ăn phù hợp chỉ trong vài giây với thuật toán tìm kiếm thông minh.",
  //     },
  //     {
  //       title: "An toàn thực phẩm",
  //       description:
  //         "Thông tin chi tiết về allergies, thành phần dinh dưỡng và hướng dẫn bảo quản thực phẩm.",
  //     },
  //     {
  //       title: "Đa dạng món ăn",
  //       description:
  //         "Từ món Việt truyền thống đến ẩm thực quốc tế, thỏa mãn mọi khẩu vị và sở thích.",
  //     },
  //     {
  //       title: "Nhanh chóng",
  //       description:
  //         "Giao diện thân thiện, dễ sử dụng với tốc độ tải nhanh và hiệu suất cao.",
  //     },
  //   ],
  //   CTA: {
  //     TITLE: "🚀 Sẵn sàng trải nghiệm?",
  //     SUBTITLE: "Tham gia cùng hàng ngàn người dùng đã tin tưởng CookMate",
  //     BUTTON: "Bắt đầu ngay hôm nay",
  //   },
  // },

  // Ingredients Input Section
  // INGREDIENTS: {
  //   TITLE: "🥘 Tìm công thức từ nguyên liệu có sẵn",
  //   SUBTITLE:
  //     "Nhập các nguyên liệu bạn có, chúng tôi sẽ gợi ý những món ăn ngon nhất cho bạn!",
  //   CARD_TITLE: "🛒 Nguyên liệu của bạn",
  //   INPUT_PLACEHOLDER: "Thêm nguyên liệu...",
  //   BUTTON_ADD: "Thêm",
  //   BUTTON_SEARCH: "🔍 Tìm công thức",
  //   SUGGESTIONS_TITLE: "💡 Gợi ý nguyên liệu phổ biến",
  //   RESULTS: {
  //     TITLE: "🍽️ Công thức phù hợp",
  //     SUBTITLE: "Dựa trên nguyên liệu của bạn",
  //     VIEW_RECIPE: "Xem công thức",
  //     DIFFICULTY_LEVELS: {
  //       EASY: "Dễ",
  //       MEDIUM: "Trung bình",
  //       HARD: "Khó",
  //     },
  //   },
  // },

  // Recipe Showcase Section
  // RECIPES: {
  //   TITLE: "🔥 Công thức nổi bật",
  //   SUBTITLE:
  //     "Khám phá những công thức được yêu thích nhất từ cộng đồng CookMate",
  //   BUTTONS: {
  //     VIEW_RECIPE: "Xem công thức",
  //     SHARE: "Chia sẻ",
  //     VIEW_MORE: "Xem thêm công thức →",
  //   },
  //   DIFFICULTY: {
  //     EASY: "Dễ",
  //     MEDIUM: "Trung bình",
  //     HARD: "Khó",
  //   },
  // },

  // Testimonials Section
  // TESTIMONIALS: {
  //   TITLE: "💬 Người dùng nói gì về CookMate",
  //   SUBTITLE:
  //     "Hơn 50,000 người dùng đã tin tưởng và sử dụng CookMate trong hành trình nấu ăn của họ",
  //   STATS: {
  //     RATING: {
  //       NUMBER: "4.8/5",
  //       LABEL: "Đánh giá trung bình",
  //     },
  //     USERS: {
  //       NUMBER: "50K+",
  //       LABEL: "Người dùng hài lòng",
  //     },
  //     RECOMMENDATION: {
  //       NUMBER: "98%",
  //       LABEL: "Tỷ lệ giới thiệu",
  //     },
  //   },
  //   CTA: {
  //     TITLE: "🎉 Tham gia cộng đồng CookMate ngay hôm nay!",
  //     SUBTITLE:
  //       "Hàng ngàn người đã tin tưởng chọn CookMate. Bạn sẽ là người tiếp theo?",
  //     FEATURES: [
  //       {
  //         TITLE: "📱 Tải app miễn phí",
  //         SUBTITLE: "iOS & Android",
  //       },
  //       {
  //         TITLE: "🎁 Thử nghiệm 30 ngày",
  //         SUBTITLE: "Hoàn toàn miễn phí",
  //       },
  //       {
  //         TITLE: "🔄 Cập nhật thường xuyên",
  //         SUBTITLE: "Tính năng mới mỗi tuần",
  //       },
  //     ],
  //   },
  // },

  // Call To Action Section
  // CTA: {
  //   TITLE: "🚀 Sẵn sàng bắt đầu hành trình nấu ăn?",
  //   SUBTITLE:
  //     "Tham gia cộng đồng CookMate ngay hôm nay và khám phá thế giới ẩm thực tuyệt vời!",
  //   EMAIL_FORM: {
  //     TITLE: "📧 Đăng ký nhận thông tin mới nhất",
  //     SUBTITLE: "Nhận những công thức mới, tips nấu ăn và ưu đãi đặc biệt",
  //     PLACEHOLDER: "Nhập email của bạn...",
  //     BUTTON: "Đăng ký ngay",
  //     SUCCESS_MESSAGE: "Cảm ơn bạn đã đăng ký!",
  //   },
  //   FEATURES: [
  //     {
  //       ICON: "🎁",
  //       TITLE: "Miễn phí",
  //       DESCRIPTION: "Sử dụng miễn phí",
  //     },
  //     {
  //       ICON: "⚡",
  //       TITLE: "Nhanh chóng",
  //       DESCRIPTION: "Kết quả tức thì",
  //     },
  //     {
  //       ICON: "🤖",
  //       TITLE: "AI thông minh",
  //       DESCRIPTION: "Gợi ý chính xác",
  //     },
  //     {
  //       ICON: "👥",
  //       TITLE: "Cộng đồng",
  //       DESCRIPTION: "Kết nối đầu bếp",
  //     },
  //   ],
  //   SOCIAL_PROOF: {
  //     TITLE: "💝 Được tin tưởng bởi",
  //     SUBTITLE: "Hàng ngàn người dùng trên toàn thế giới",
  //   },
  // },

  // Footer
  FOOTER: {
    TAGLINE: "Nấu ăn thông minh cùng AI",
    DESCRIPTION:
      "CookMate - Ứng dụng nấu ăn thông minh với AI, giúp bạn tìm kiếm công thức phù hợp từ nguyên liệu có sẵn.",
    LINKS: {
      ABOUT: "Về chúng tôi",
      FEATURES: "Tính năng",
      RECIPES: "Công thức",
      BLOG: "Blog",
      CONTACT: "Liên hệ",
      PRIVACY: "Chính sách bảo mật",
      TERMS: "Điều khoản",
    },
    CONTACT: {
      EMAIL: "contact@cookmate.com",
      PHONE: "+84 123 456 789",
      ADDRESS: "123 Đường ABC, Quận 1, TP.HCM",
    },
    SOCIAL: {
      FACEBOOK: "Facebook",
      INSTAGRAM: "Instagram",
      YOUTUBE: "YouTube",
      TWITTER: "Twitter",
    },
    COPYRIGHT: "© 2024 CookMate. Tất cả quyền được bảo lưu.",
  },

  // Common
  COMMON: {
    BUTTONS: {
      LEARN_MORE: "Tìm hiểu thêm",
      GET_STARTED: "Bắt đầu",
      VIEW_ALL: "Xem tất cả",
      BACK: "Quay lại",
      NEXT: "Tiếp theo",
      SUBMIT: "Gửi",
      CANCEL: "Hủy",
      SAVE: "Lưu",
      EDIT: "Chỉnh sửa",
      DELETE: "Xóa",
    },
    MESSAGES: {
      LOADING: "Đang tải...",
      ERROR: "Có lỗi xảy ra",
      SUCCESS: "Thành công!",
      NO_DATA: "Không có dữ liệu",
    },
  },
};

// Export individual sections for easier imports
export const {
  APP_NAME,
  HEADER,
  HERO,
  FEATURES,
  INGREDIENTS,
  RECIPES,
  TESTIMONIALS,
  CTA,
  FOOTER,
  COMMON,
} = APP_CONTENT;
